 ...
 * This file contains code implementing Algorithm 4.10 of
 ...
 * Each thread produces partial sum. Sums are then 
 * aggregated sequentially.
 */
 #include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
float piEst(int n){
	...
	float *psum = calloc(nt, sizeof(float));
	#pragma omp parallel
	{
		float localSum = 0;
		...
		//could alternatively add localSums using critical section
		psum[id] = localSum;
	}
	for(int i=0; i<nt; i++)
		sum +=psum[i];
